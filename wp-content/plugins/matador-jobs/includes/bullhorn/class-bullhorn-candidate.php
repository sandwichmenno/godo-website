...
class Bullhorn_Candidate extends Bullhorn_Connection {
		...
		if ( ! empty( $bullhorn_skills ) ) {
			foreach ( $candidate_skills as $skill ) {
				...
				if ( $key ) {
					...
					$candidate_categories_ids[] = $skill;
				}
			}
	...
	public function save_candidate_specialties( $candidate = null ) {
		...
		if ( ! empty( $bullhorn_specialties ) ) {
			foreach ( $candidate_specialties as $category ) {
				$key = array_search( strtolower( $category ), $bullhorn_specialties, true );
				if ( $key ) {
					$candidate_specialties_ids[] = $key;
				} elseif ( array_key_exists( $category ,$bullhorn_specialties ) ) {
					$candidate_specialties_ids[] = $category;
				}
			}
	...
	public function save_candidate_business_sectors( $candidate = null ) {
		...
		$candidate_business_sectors     = $candidate->categories; // @codingStandardsIgnoreLine (SnakeCase)

		$candidate_business_sectors_ids = array();

		if ( ! empty( $bullhorn_business_sectors ) ) {
			foreach ( $candidate_business_sectors as $category ) {
				$key = array_search( strtolower( $category ), $bullhorn_business_sectors, true );
				if ( $key ) {
					$candidate_business_sectors_ids[] = $key;
				} elseif ( array_key_exists( $category ,$bullhorn_business_sectors ) ) {
					$candidate_business_sectors_ids[] = $category;
				}
			}
		...
		if ( ! $categories ) {
			...
			if ( isset( $request->data ) ) {
				...
				foreach ( $request->data as $category ) {
					$categories[ $category->value ] = strtolower( trim( $category->label ) );
				}

				$skill_list = array_unique( $categories );
				set_transient( $transient, $skill_list, HOUR_IN_SECONDS * 6 );
			}
		}
		...
		if ( ! $specialties ) {
			...
			if ( isset( $request->data ) ) {
				...
				foreach ( $request->data as $specialty ) {
					$specialties[ $specialty->value ] = strtolower( trim( $specialty->label ) );
				}

				$skill_list = array_unique( $specialties );
				set_transient( $transient, $skill_list, HOUR_IN_SECONDS * 6 );
			}
		}
		...
		if ( ! $business_sectors ) {
			...
			if ( isset( $request->data ) ) {
				...
				foreach ( $request->data as $business_sector ) {
					$business_sectors[ $business_sector->value ] = strtolower( trim( $business_sector->label ) );
				}

				$skill_list = array_unique( $business_sectors );
				set_transient( $transient, $skill_list, HOUR_IN_SECONDS * 6 );
			}
		}
