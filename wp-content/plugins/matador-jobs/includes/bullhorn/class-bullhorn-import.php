	...
	private function save_jobs( $jobs, $existing ) {
		...
		if ( ! empty( $jobs ) ) {
			...
			foreach ( $jobs as $index => $job ) {
				 ...
				 *
				 * Filter to control in a job should overwritten.
				 *
				 * @since 3.5.0
				 *
				 ...
				 */
				if ( $wpid && ! ( apply_filters( 'matador_bullhorn_import_overwrite_job_on_sync', true, $job, $wpid ) ) ) {
				...
				if ( ! is_wp_error( $wpid ) ) {
					...
					new Event_Log( 'bullhorn-import-save-job-action', esc_html( sprintf( __( 'Running action: matador_bullhorn_import_save_job for Job #%1$s.', 'bh-staffing-job-listing-and-cv-upload-for-wp' ), $job->id ) ) );
				...
				// Translators: Placeholder is Job ID number.
				new Event_Log( 'bullhorn-import-job-complete', esc_html( sprintf( __( 'Bullhorn Job #%1$s has been imported.', 'bh-staffing-job-listing-and-cv-upload-for-wp' ), $job->id ) ) );
			}
